...
#   ⟨46⟩ ARC-AGI-3 `su15` — Agent Trajectories (`codex-gpt5.5`) 
Gameplay trajectories from the harness×model pair `codex-gpt5.5` playing the ARC-AGI-3⟨47⟩ game `su15`, part of the ARA-as-world-model generalization experiment. The agent builds a structured world model (an Agent-Native Research Artifact) live during play and consults it to crack levels it cannot solve from cold exploration.
