...
missing values use`set_index` after reading the data instead of`index_col` .When `header` is a list of two or more rows (`MultiIndex` columns), the row after the last header row is expected to hold the
index name(s) — blank if the index is unnamed — as written by`to_excel` . A file lacking this separator row can lose its first
data row to the index name(s); see the user guide for details.
 ...
 Returns a subset of the columns according to behavior above. Element order is ignored, so `usecols=[0, 1]` is the same as`[1, 0]` .
To instantiate a`DataFrame` with element order preserved
use`pd.read_excel(path, usecols=['foo', 'bar'])[['foo', 'bar']]` for
columns in`['foo', 'bar']` order or`pd.read_excel(path, usecols=['foo', 'bar'])[['bar', 'foo']]` for`['bar', 'foo']` order.
