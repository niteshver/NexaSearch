# pandas.api.typing.SeriesGroupBy.max#

- 
SeriesGroupBy.max(*numeric_only=False* ,*min_count=-1* ,*skipna=True* ,*engine=None* ,*engine_kwargs=None* )[source]#
- Compute max of group values. Returns the maximum value for each group. Missing values are excluded by default but this behavior can be controlled with the `skipna` parameter.
  - Parameters:
