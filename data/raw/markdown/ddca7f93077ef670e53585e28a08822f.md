...
```
In [11]: f'dense: {df.memory_usage().sum()} bytes'
Out[11]: 'dense: 320132 bytes'
In [12]: f'sparse: {sdf.memory_usage().sum()} bytes'
Out[12]: 'sparse: 228 bytes'
```
...
Out[17]: 
<SparseArray>
[-1.9556635297215477, -1.6588664275960427,                 nan,
                 nan,                 nan,  1.1589328886422277,
 0.14529711373305043,                 nan,  0.6060271905134522,
  1.3342113401317768]
Length: 10, dtype: Sparse[float64, nan]
```
...
Out[20]: Sparse[datetime64[ns], np.datetime64('NaT','ns')]
```
...
<SparseArray>
[1, 1, 1, 2.0, 1]
Length: 5, dtype: Sparse[float64, 1]
...
To convert a `DataFrame` back to a sparse SciPy matrix in COO format, you can use the `DataFrame.sparse.to_coo()` method:
