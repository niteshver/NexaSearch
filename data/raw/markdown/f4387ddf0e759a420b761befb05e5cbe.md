...
- Purely integer-location based indexing for selection by position. Deprecated since version 2.2.0: Returning a tuple from a callable is deprecated. `.iloc[]` is primarily integer position based (from`0` to`length-1` of the axis), but may also be used with a boolean
