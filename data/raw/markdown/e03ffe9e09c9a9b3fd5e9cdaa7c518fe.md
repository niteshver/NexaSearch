...
- **Lack of support for multiple engines.** The current pandas I/O API
  ...
  are likely to have different parameters and using`*args` and`**kwargs` provides users with a more complex and difficult experience. For this
...
- **No improvements to the current I/O API** . In the discussions of this
  ...
  fix the inconsistency of using`read` /`to` (instead of for example`read` /`write` ), avoid using`to_` prefixed methods for non-I/O
...
- 
Discussing a better API for pandas connectors. For example, using `read_*` methods instead of`from_*` methods, renaming`to_*` methods not used as I/O
...
- 
Implement as I/O connectors some of the formats supported by the `DataFrame` constructor.
