This is documentation for an old release of Scikit-learn (version 0.17). Try the latest stable release (version 1.9) or development (unstable) versions.
...
`LinearSVC` take as input two arrays: an array X of size ```
[n_samples,
n_features]
```
 holding the training samples, and an array y of class labels
