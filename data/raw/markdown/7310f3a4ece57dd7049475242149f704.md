...
If you would like to receive regular daily listings of the abstracts of new submissions by email, then you can subscribe to this service as follows. Note that the email must be sent as **plain ASCII txt**. Richtext format emails will be ignored by the system, as will UTF-8 characters within the `Subject` field. 
  ...
  2. **For archives divided into subject classes:** If the archive to which you are subscribing requires distinct subject classes (such as the `math`, `physics`, `cs`, `q-bio` and `q-fin` archives; but _not`astro-ph` , `cond-mat` or `nlin`_, see handling subscriptions to all physics archives through physics below), then you (un)subscribe to a specific category, indicating the categories in the body of the message. Here is an example
...
```
  ...
  1. **For archives not divided into subject classes:** Send an e-mail message to the archive(s) of interest, in the following form
...
If you want to cancel your subscription to a particular subject class in an archive that supports subject classes, then see the example use of in the **`subscribe`**command above.
