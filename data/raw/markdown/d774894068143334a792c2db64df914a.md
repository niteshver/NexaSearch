 ...
 **sample_weight** : array-like of shape = [n_samples], optionalSample weights. Returns: **average_precision** : floatSee also 
  - `roc_auc_score`
  - Area under the ROC curve
  - `precision_recall_curve`
  - Compute precision-recall pairs for different probability thresholds
 References [R157] Wikipedia entry for the Average precision Examples >>> import numpy as np >>> from sklearn.metrics import average_precision_score >>> y_true = np.array([0, 0, 1, 1]) >>> y_scores = np.array([0.1, 0.4, 0.35, 0.8]) >>> average_precision_score(y_true, y_scores) 0.79...