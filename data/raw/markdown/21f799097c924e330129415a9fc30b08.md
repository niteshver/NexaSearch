...
| `BaseOffset` | Base class for all pandas date offsets. | 

### Properties#

| `BaseOffset.freqstr` | Return a string representing the frequency. | 
| `BaseOffset.kwds` | Return a dict of extra parameters for the offset. | 
| `BaseOffset.nanos` | Return an integer of the total number of nanoseconds. | 
| `BaseOffset.normalize` | Return boolean whether the frequency can align with midnight. | 
| `BaseOffset.rule_code` | Return a string representing the base frequency. | 
| `BaseOffset.n` | Return the count of the number of periods. | 

### Methods#

| `BaseOffset.copy` () | Return a copy of the frequency. | 
| `BaseOffset.is_on_offset` (dt) | Return boolean whether a timestamp intersects with this frequency. | 
| `BaseOffset.is_month_start` (ts) | Return boolean whether a timestamp occurs on the month start. | 
| `BaseOffset.is_month_end` (ts) | Return boolean whether a timestamp occurs on the month end. | 
| `BaseOffset.is_quarter_start` (ts) | Return boolean whether a timestamp occurs on the quarter start. | 
| `BaseOffset.is_quarter_end` (ts) | Return boolean whether a timestamp occurs on the quarter end. | 
| `BaseOffset.is_year_start` (ts) | Return boolean whether a timestamp occurs on the year start. | 
| `BaseOffset.is_year_end` (ts) | Return boolean whether a timestamp occurs on the year end. | 
| `BaseOffset.rollback` (dt) | Roll provided date backward to next offset only if not on offset. | 
| `BaseOffset.rollforward` (dt) | Roll provided date forward to next offset only if not on offset. | 
...
| `CustomBusinessDay` ([n, normalize, weekmask, ...]) | DateOffset subclass representing possibly n custom business days. | 
...
| `BQuarterEnd.startingMonth` | Return the month of the year that anchors the quarters. | 
...
| `BQuarterBegin.startingMonth` | Return the month of the year that anchors the quarters. | 
...
| `QuarterEnd.startingMonth` | Return the month of the year that anchors the quarters. | 
...
| `QuarterBegin.startingMonth` | Return the month of the year that anchors the quarters. | 
...
| `BHalfYearEnd.startingMonth` | Return the month of the year that anchors the half-years. | 
...
| `BHalfYearBegin.startingMonth` | Return the month of the year that anchors the half-years. | 
...
| `HalfYearEnd.startingMonth` | Return the month of the year that anchors the half-years. | 
...
| `HalfYearBegin.startingMonth` | Return the month of the year that anchors the half-years. | 
...
| `Holiday.dates` (start_date, end_date[, ...]) | Calculate holidays observed between start date and end date. | 

## Calendars#

| `AbstractHolidayCalendar` ([name, rules]) | Abstract interface to create holidays following certain rules. | 
| `USFederalHolidayCalendar` ([name, rules]) | US Federal Government holiday calendar. | 

### Methods#

| `AbstractHolidayCalendar.holidays` ([start, ...]) | Return a curve with holidays between start_date and end_date. | 
| `AbstractHolidayCalendar.merge` (other[, inplace]) | Merge holiday calendars together. | 
| `AbstractHolidayCalendar.merge_class` (base, other) | Merge holiday calendars together. | 
| `AbstractHolidayCalendar.rule_from_name` (name) | Return the rule for the holiday with the given name. | 

## Observance rules#

| `after_nearest_workday` (dt) | Move to the workday after the nearest workday. | 
| `before_nearest_workday` (dt) | Move to the workday before the nearest workday. | 
| `nearest_workday` (dt) | Move Saturday to Friday and Sunday to Monday. | 
| `next_monday` (dt) | Move Saturday and Sunday to the following Monday. | 
| `next_monday_or_tuesday` (dt) | Move Saturday to Monday, and Sunday or Monday to Tuesday. | 
| `next_workday` (dt) | Move to the next weekday. | 
| `previous_friday` (dt) | Move Saturday and Sunday back to the previous Friday. | 
| `previous_workday` (dt) | Move to the previous weekday. | 
| `sunday_to_monday` (dt) | Move Sunday to the following Monday. | 
| `weekend_to_monday` (dt) | Move Saturday and Sunday to the following Monday. | 

## Calendar utilities#

| `HolidayCalendarFactory` (name, base, other[, ...]) | Create a holiday calendar class from two sets of rules. | 
| `get_calendar` (name) | Return an instance of a calendar based on its name. | 
| `register` (cls) | Add a holiday calendar class to the registry of named calendars. |