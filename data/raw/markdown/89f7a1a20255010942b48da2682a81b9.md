...
If you’re building a library that implements the interface, please publicize it on the ecosystem page.
...
We provide a test suite for ensuring that your extension arrays satisfy the expected behavior. To use the test suite, you must provide several pytest fixtures and inherit from the base test class. The required fixtures are found in pandas-dev/pandas.
...
class TestConstructors(base.BaseConstructorsTests):
    pass
```
See pandas-dev/pandas for a list of all the tests available.
...
```
More information on how to implement a third-party plotting backend can be found at pandas-dev/pandas.
...
semantics, arithmetic methods on `DataFrame`, `Series`, and `Index`
...
The `__pandas_priority__` of `DataFrame`, `Series`, and `Index` are `4000`, `3000`, and `2000` respectively.  The base `ExtensionArray.__pandas_priority__` is `1000`.
