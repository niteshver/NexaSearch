...
dtypes).
 ...
 Notes The mask method is an application of the if-then idiom. For each element in the calling DataFrame, if `cond` is`False` the
element is used; otherwise the corresponding element from the DataFrame`other` is used. If the axis of`other` does not align with axis of`cond` Series/DataFrame, the misaligned index positions will be filled with
True.The signature for `DataFrame.where()` differs from`numpy.where()` . Roughly`df1.where(m, df2)` is equivalent to`np.where(m, df1, df2)` .For further details and examples see the `mask` documentation in
