...
Rolling objects are returned by `.rolling` calls: `pandas.DataFrame.rolling()` and `pandas.Series.rolling()`.
Expanding objects are returned by `.expanding` calls: `pandas.DataFrame.expanding()` and `pandas.Series.expanding()`.
ExponentialMovingWindow objects are returned by `.ewm` calls: `pandas.DataFrame.ewm()` and `pandas.Series.ewm()`.
