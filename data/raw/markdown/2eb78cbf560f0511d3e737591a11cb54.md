...
| `Series.backfill` (*[, axis, inplace, limit, ...]) | (DEPRECATED) Synonym for `DataFrame.fillna()` with`method='bfill'` . | 
...
| `Series.pad` (*[, axis, inplace, limit, downcast]) | (DEPRECATED) Synonym for `DataFrame.fillna()` with`method='ffill'` . | 
...
| `Series.ravel` ([order]) | Return the flattened underlying data as an ndarray or ExtensionArray. | 
