...
- A multi-level, or hierarchical, index object for pandas objects. A MultiIndex allows indexing with multiple keys, effectively enabling multiple dimensions of indexing on a single axis. It can be thought of as an array of tuples where each tuple is unique. 
  - Parameters:
    - **levels** sequence of arrays
    - The unique labels for each level. Uniqueness is determined by equality, so values that compare equal (e.g. `1` ,`1.0` and`True` ) cannot
