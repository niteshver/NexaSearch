...
Kanisia⟨28⟩
/
hdf5-external-file-read-modelscan-bypass⟨29⟩
like 0
...
Check out the documentation⟨34⟩ for more information.
  * HDF5 External Dataset File Read ModelScan Bypass⟨35⟩
    * Target⟨36⟩
    * Summary⟨37⟩
    * Files⟨38⟩
    * Reproduction⟨39⟩
...
#   ⟨35⟩ HDF5 External Dataset File Read ModelScan Bypass 
##   ⟨36⟩ Target 
HDF5 model format.
##   ⟨37⟩ Summary 
...
##   ⟨38⟩ Files 
  * `hdf5_external_file_read.h5`: PoC model file to upload to Hugging Face.
  * `secret.txt`: Reproduction fixture read through the HDF5 external dataset.
  * `build_and_verify.py`: Builds the HDF5 file and verifies metadata, external-file read behavior, and ModelScan result.
...
Expected result:
  ...
  * dataset external storage metadata is `["secret.txt", 0, 32]`.
  * reading `h5["weights"][:]` returns `HDF5_EXTERNAL_FILE_READ_MARKER`.
