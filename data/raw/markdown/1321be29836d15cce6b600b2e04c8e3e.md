...
time spans and performing**calendar logic** ,
...
- Enhanced support for time zones. Add `tz_convert` and`tz_localize` methods to TimeSeries and DataFrame. All
...
functions
- Robust **frequency inference** function`infer_freq` and`inferred_freq` property of DatetimeIndex, with option to infer frequency on construction of
DatetimeIndex
...
- New cut and `qcut` functions (like R’s cut
