...
- The `year` column of`DatetimeIndex.isocalendar()` and`Series.dt.isocalendar()` is now`Int32` instead of`UInt32` , so that years before year 1 are representable (GH 66549)
...
- Deprecated the `lsuffix` and`rsuffix` keywords in`DataFrame.join()` in favor of`suffixes` , consistent with`merge()` and`DataFrame.merge()` (GH 46046)
...
- Deprecated `inplace` keyword for`DataFrame.rename()` ,`DataFrame.drop()` ,`DataFrame.dropna()` ,`DataFrame.drop_duplicates()` ,`DataFrame.sort_values()` ,`DataFrame.sort_index()` ,`DataFrame.eval()` ,`DataFrame.query()` (and the`Series` equivalents). This keyword will be removed in a future version (GH 63207); see also PDEP-8
...
- Bug in `DatetimeIndex.year` and`Series.dt.year` silently returning a wrapped year, e.g.`219250468` for year`292277026596` , for second-resolution datetimes with a year beyond`2**31` ; these now raise`ValueError` , and`DatetimeIndex.is_leap_year` no longer takes the leap-year status from the wrapped year (GH 66549)
...
- Bug in `DatetimeIndex.isocalendar()` and`Series.dt.isocalendar()` returning a wrapped year for dates before year 1, e.g.`4294967196` for year`-100` ; a year beyond`2**31` now raises`ValueError` instead of being silently truncated (GH 66549)
...
- Bug in `day_of_year` and`week` , on`Timestamp` as well as on`DatetimeIndex` and`Series` , taking the leap-year status from a wrapped year for second-resolution datetimes with a year beyond`2**31` (GH 66549)
...
- `read_csv()` ,`read_fwf()` , and`read_excel()` with a numeric`ArrowDtype` (e.g.`dtype="double[pyarrow]"` ) now raise`ValueError` for an empty field that is not treated as missing (e.g. with`keep_default_na=False` ), matching the behavior of numpy dtypes such as`float64` ; previously the value was silently stored as`NaN` but was not considered missing by`Series.isna()` and`Series.fillna()` (GH 66834)
...
- Fixed bug in `read_csv()` with`engine="pyarrow"` where a requested dtype whose conversion must start from the original text was applied only after pyarrow’s type inference, giving results that differed from the other engines: values with leading zeros lost the zeros under a string dtype,`dtype=object` produced the inferred values instead of the raw strings, and`dtype="category"` produced inferred-value categories instead of string categories. Such columns are now parsed as strings. For a scalar`dtype` (e.g.`dtype=str` ) this additionally requires pyarrow 25.0 or newer (GH 9435, GH 57666, GH 58260)
