...
- Iterate over DataFrame rows as namedtuples. Each row becomes a namedtuple (or plain tuple if `name` is
None) with field names taken from the column names or
positional names. Generally faster and more type-stable than`iterrows()` .
  - Parameters:
