...
data rather than the first line of the file.When inferred from the file contents, headers are kept distinct from each other by renaming duplicate names with a numeric suffix of the form `".{count}"` starting from 1, e.g.`"foo"` and`"foo.1"` .
...
custom compression dictionary:`compression={'method': 'zstd', 'dict_data': my_compression_dict}` .
    ...
    - 2 or csv.QUOTE_NONNUMERIC, 3 or csv.QUOTE_NONE}, default csv.QUOTE_MINIMAL Control field quoting behavior per `csv.QUOTE_*` constants. Default is`csv.QUOTE_MINIMAL` (i.e., 0) which implies that
