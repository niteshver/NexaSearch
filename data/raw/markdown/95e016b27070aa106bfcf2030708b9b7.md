...
based hierarchical indexes.As using integer slices with `.ix` have different behavior depending on whether the slice
is interpreted as position based or label based, it’s usually better to be
explicit and use`.iloc` or`.loc` .See more at Advanced Indexing and Advanced Hierarchical.
...
dtypes, they *WILL* be respected, however (GH 2837)
...
Bottleneck is now a Recommended Dependencies, to accelerate certain
...
provided to support iteration on `select` and `select_as_multiple` (GH 3076)
...
instead of Options.get_xxx_data() (GH 2758).
...
elements printed per sequence pprinting it.  (GH 2979)
...
calculated for frames above 1M rows (configurable). (GH 2807, GH 2918)
...
for plots. Based on https://gist.github.com/huyng/816622 (GH 3075).
