...
- Return boolean Series denoting duplicate rows. Rows are considered duplicated when the same values appear more than once across the requested columns. Considering certain columns is optional. 
  - Parameters:
  ...
  - Returns:
    - Series
    - Boolean Series indicating whether each row is a duplicated row according to `keep` .
 See also 
  - `Index.duplicated`
