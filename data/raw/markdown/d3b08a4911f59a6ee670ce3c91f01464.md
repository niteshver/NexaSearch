...
- Call `func` on self producing a DataFrame with the same axis shape as self.Unlike aggregation, transformation preserves the shape of the input. The provided function must return a result that is the same size as the input along the specified axis, raising a `ValueError` otherwise.
  - Parameters:
    - **func** function, str, list-like or dict-like
    - Function to use for transforming the data. If a function, must either work when passed a DataFrame or when passed to DataFrame.apply. If func is both list-like and dict-like, dict-like behavior takes precedence. Accepted combinations are: 
      - function
