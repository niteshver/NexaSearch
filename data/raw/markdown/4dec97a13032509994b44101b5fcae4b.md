...
| `DataFrame.backfill` (*[, axis, inplace, ...]) | (DEPRECATED) Synonym for `DataFrame.fillna()` with`method='bfill'` . | 
...
| `DataFrame.pad` (*[, axis, inplace, limit, ...]) | (DEPRECATED) Synonym for `DataFrame.fillna()` with`method='ffill'` . | 
