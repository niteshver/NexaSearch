...
Out[69]: <pandas.api.typing.SeriesGroupBy object at 0x7f9b0b844890>
```
...
The `aggregate()` method can accept many different types of
...
`aggregate()`. Users are encouraged to use the shorthand,
...
provided `Series`. See Mutating with User Defined Function (UDF) methods for more information.
...
- The values are tuples whose first element is the column to select and the second element is the aggregation to apply to that column. pandas provides the `NamedAgg` namedtuple with the fields`['column', 'aggfunc']` to make it clearer what the arguments are. As usual, the aggregation can
...
`transform()` (see the next section) will broadcast the result
...
`transform()` method can accept string aliases to the built-in
...
leopard      138.2
```
In addition to string aliases, the `transform()` method can
