...
For an introduction to using DataFrames, see Essential basic functionality.
...
| `DataFrame.__invert__` () | Return the bitwise inverse of the Series/DataFrame, element-wise. | 
...
| `DataFrame.__dataframe__` ([nan_as_null, ...]) | (DEPRECATED) Return the dataframe interchange object implementing the interchange protocol. |