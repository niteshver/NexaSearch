...
Pyarrow-backed types below need to be passed into `ArrowDtype` to be recognized by pandas e.g. `pd.ArrowDtype(pa.bool_())`.
...
`pd.StringDtype("pyarrow")` is described below in the string section
...
| `Timestamp.dayofweek` | (DEPRECATED) Return day of the week. | 
| `Timestamp.day_of_week` | Return day of the week. | 
| `Timestamp.dayofyear` | (DEPRECATED) Return day of the year. | 
| `Timestamp.day_of_year` | Return the day of the year. | 
| `Timestamp.days_in_month` | Return the number of days in the month. | 
| `Timestamp.daysinmonth` | (DEPRECATED) Return the number of days in the month. | 
...
| `Timestamp.fromisocalendar` (year, week, day) | Construct a Timestamp from an ISO year, week number, and weekday. | 
| `Timestamp.fromisoformat` (date_string) | Construct a Timestamp from a string in ISO 8601 format. | 
...
| `Timestamp.tz_localize` (tz[, ambiguous, ...]) | Attach or detach a time zone on a Timestamp. | 
...
| `Timestamp.utcnow` () | (DEPRECATED) Return a new Timestamp representing UTC day and time. | 
...
for symmetry with `Timestamp`. `NaT`
...
| `Period.dayofweek` | (DEPRECATED) Return day of the week. | 
| `Period.day_of_week` | Day of the week the period lies in, with Monday=0 and Sunday=6. | 
| `Period.dayofyear` | (DEPRECATED) Return day of the year. | 
...
| `Period.daysinmonth` | (DEPRECATED) Get the total number of days of the month that this period falls on. | 
...
| `Period.strftime` (fmt) | (DEPRECATED) Returns a formatted string representation of the `Period` . | 
