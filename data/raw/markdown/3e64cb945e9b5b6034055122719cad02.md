...
4. If not a release candidate, make sure all backporting pull requests to the branch being released are merged, and no merged pull requests are missing a backport (check the [“Still Needs Manual Backport”](pandas-dev/pandas) label for this).
...
2. Download the source distribution and wheels from the wheel staging area. Be careful to make sure that no wheels are missing (e.g. due to failed builds).
