...
File ~/work/pandas/pandas/pandas/core/reshape/merge.py:1028, in _MergeOperation.__init__(self, left, right, how, on, left_on, right_on, left_index, right_index, sort, suffixes, indicator, validate)
   1024 # If argument passed to validate,
   1025 # check if columns specified as unique
   1026 # are in fact unique.
   1027 if validate is not None:
-> 1028     self._validate_validate_kwd(validate)
File ~/work/pandas/pandas/pandas/core/reshape/merge.py:1988, in _MergeOperation._validate_validate_kwd(self, validate)
   1983         raise MergeError(
   1984             "Merge keys are not unique in left dataset; not a one-to-one merge"
   1985             f"{left_error_msg(left_join_index)}"
   1986         )
   1987     if not right_unique:
-> 1988         raise MergeError(
   1989             "Merge keys are not unique in right dataset; not a one-to-one merge"
   1990             f"{right_error_msg(right_join_index)}"
   1991         )
   1993 elif validate in ["one_to_many", "1:m"]:
   1994     if not left_unique:
...
the last row in the `right` `DataFrame` is selected where the `on` key is less
