...
File ~/work/pandas/pandas/pandas/core/reshape/merge.py:388, in merge(left, right, how, on, left_on, right_on, left_index, right_index, sort, suffixes, copy, indicator, validate)
    374     return _cross_merge(
    375         left_df,
    376         right_df,
   (...)    385         validate=validate,
    386     )
    387 else:
--> 388     op = _MergeOperation(
    389         left_df,
    390         right_df,
    391         how=how,
    392         on=on,
    393         left_on=left_on,
    394         right_on=right_on,
    395         left_index=left_index,
    396         right_index=right_index,
    397         sort=sort,
    398         suffixes=suffixes,
    399         indicator=indicator,
    400         validate=validate,
    401     )
    402     return op.get_result()
File ~/work/pandas/pandas/pandas/core/reshape/merge.py:1030, in _MergeOperation.__init__(self, left, right, how, on, left_on, right_on, left_index, right_index, sort, suffixes, indicator, validate)
   1026 # If argument passed to validate,
   1027 # check if columns specified as unique
   1028 # are in fact unique.
   1029 if validate is not None:
-> 1030     self._validate_validate_kwd(validate)
File ~/work/pandas/pandas/pandas/core/reshape/merge.py:2003, in _MergeOperation._validate_validate_kwd(self, validate)
   1998         raise MergeError(
   1999             "Merge keys are not unique in left dataset; not a one-to-one merge"
   2000             f"{left_error_msg(left_join_index)}"
   2001         )
   2002     if not right_unique:
-> 2003         raise MergeError(
   2004             "Merge keys are not unique in right dataset; not a one-to-one merge"
   2005             f"{right_error_msg(right_join_index)}"
   2006         )
   2008 elif validate in ["one_to_many", "1:m"]:
   2009     if not left_unique:
...
A list or tuple of `DataFrame` can also be passed to `join()`
...
the last row in the `right` `DataFrame` is selected where the `on` key is
less than or equal to the left’s key. Both `DataFrame` must be sorted by the
key.
