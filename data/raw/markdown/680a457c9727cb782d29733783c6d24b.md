...
Copy to bucket new
  * DSC 370M (FineWeb-Edu 1B) — v3, leak-fixed⟨46⟩
    * 1. What is DSC?⟨47⟩
    * 2. What's new in v3⟨48⟩
    * 3. RULER results vs vanilla GDN-2 1B⟨49⟩
    * 4. Known limitations⟨50⟩
    * 5. Citation⟨51⟩


#   ⟨46⟩ DSC 370M (FineWeb-Edu 1B) — v3, leak-fixed   
|   |   |  
| --- | --- |  
...
**v1 leak mechanism** (patched in v3):
  * `seg_query` was computed from chunk-mean of the _current_ chunk, allowing future positions inside chunk_i to influence chunk_i's routing query
...
**v3 fix** :
  ...
  * `seg_query[:, 1:] = normed_chunks[:, :-1, -1]` (use last position of _previous_ chunk as the query for routing decision of current chunk)
  * Causal mask = `torch.triu(ones(N, N, dtype=bool), diagonal=0)` — strict upper triangular including diagonal, so future chunks are fully masked
...
##   ⟨49⟩ 3. RULER results vs vanilla GDN-2 1B 
...
##   ⟨50⟩ 4. Known limitations 
  * Trained on 1B tokens only — research-grade, not production-ready
