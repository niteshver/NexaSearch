...
is not like-indexed with respect to the input.Changed in version 1.5.0: Warns that group_keys will no longer be ignored when the result from `apply` is a like-indexed Series or DataFrame.
Specify`group_keys` explicitly to include the group keys or
not.
    - **squeeze** bool, default False
    - Reduce the dimensionality of the return type if possible, otherwise return a consistent type. Deprecated since version 1.1.0.
