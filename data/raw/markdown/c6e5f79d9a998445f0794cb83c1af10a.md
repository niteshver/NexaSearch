...
File ~/work/pandas/pandas/pandas/core/dtypes/concat.py:343, in union_categoricals(to_union, sort_categories, ignore_order)
    341     if all(c.ordered for c in to_union):
    342         msg = "to union ordered Categoricals, all categories must be the same"
--> 343         raise TypeError(msg)
    344     raise TypeError("Categorical.ordered must be the same")
    346 if ignore_order:
...
```
In [245]: s = pd.Series(["foo", "bar"] * 1000, dtype="object")
# object dtype
In [246]: s.memory_usage(deep=True)
Out[246]: 104132
# category dtype
In [247]: s.astype("category").memory_usage(deep=True)
Out[247]: 2155
```
Note
...
```
In [248]: s = pd.Series(["foo%04d" % i for i in range(2000)], dtype="object")
# object dtype
In [249]: s.memory_usage(deep=True)
Out[249]: 112132
# category dtype
In [250]: s.astype("category").memory_usage(deep=True)
Out[250]: 34382
```
