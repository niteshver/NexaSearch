...
- A string variable consisting of only a few different values. Converting such a string variable to a categorical variable will save some memory, see section Memory usage.
- The lexical order of a variable is not the same as the logical order (“one”, “two”, “three”). By converting to a categorical and specifying an order on the categories, sorting and min/max will use the logical order instead of the lexical order, see section Sorting and order.
...
In [75]: s = s.cat.rename_categories({"Group a": "x", "Group b": "y", "Group c": "z"})
In [76]: s
Out[76]: 
0    x
1    y
...
Out[80]: Index(['x', 'y', 'z', 4], dtype='object')
In [81]: s
Out[81]: 
0    x
1    y
...
4   2015-01-05
dtype: category
Categories (5, datetime64[us]): [2015-01-01, 2015-01-02, 2015-01-03, 2015-01-04,
                                 2015-01-05]
...
Column assignment vs. in-place setting for details.
...
1    4.0
dtype: float64
In [196]: pd.concat([s1, s3], union_categories=True)
Out[196]: 
0    a
1    b
0    b
...
dtype: category
Categories (3, str): ['a', 'b', 'c']
In [197]: pd.concat([int_cats, float_cats], union_categories=True)
Out[197]: 
0    1.0
1    2.0
0    3.0
...
In [198]: pd.concat([s1, s3]).astype("category")
Out[198]: 
0    a
1    b
0    b
...
dtype: category
Categories (3, str): ['a', 'b', 'c']
In [199]: union_categoricals([s1.array, s3.array])
Out[199]: 
['a', 'b', 'b', 'c']
Categories (3, str): ['a', 'b', 'c']
```
...
```
In [200]: from pandas.api.types import union_categoricals
In [201]: a = pd.Categorical(["b", "c"])
In [202]: b = pd.Categorical(["a", "b"])
...
```
In [204]: union_categoricals([a, b], sort_categories=True)
Out[204]: 
['b', 'c', 'a', 'b']
Categories (3, str): ['a', 'b', 'c']
```
...
```
In [205]: a = pd.Categorical(["a", "b"], ordered=True)
In [206]: b = pd.Categorical(["a", "b", "a"], ordered=True)
In [207]: union_categoricals([a, b])
Out[207]: 
['a', 'b', 'a', 'b', 'a']
Categories (2, str): ['a' < 'b']
```
...
```
In [208]: a = pd.Categorical(["a", "b"], ordered=True)
In [209]: b = pd.Categorical(["a", "b", "c"], ordered=True)
In [210]: union_categoricals([a, b])
---------------------------------------------------------------------------
TypeError                                 Traceback (most recent call last)
Cell In[210], line 1
----> 1 union_categoricals([a, b])
File ~/work/pandas/pandas/pandas/core/dtypes/concat.py:393, in union_categoricals(to_union, sort_categories, ignore_order)
    391     if all(c.ordered for c in to_union):
    392         msg = "to union ordered Categoricals, all categories must be the same"
--> 393         raise TypeError(msg)
    394     raise TypeError("Categorical.ordered must be the same")
    396 if ignore_order:
...
```
In [211]: a = pd.Categorical(["a", "b", "c"], ordered=True)
In [212]: b = pd.Categorical(["c", "b", "a"], ordered=True)
In [213]: union_categoricals([a, b], ignore_order=True)
Out[213]: 
['a', 'b', 'c', 'c', 'b', 'a']
Categories (3, str): ['a', 'b', 'c']
```
...
```
In [214]: a = pd.Series(["b", "c"], dtype="category")
In [215]: b = pd.Series(["a", "b"], dtype="category")
In [216]: union_categoricals([a, b])
Out[216]: 
['b', 'c', 'a', 'b']
Categories (3, str): ['b', 'c', 'a']
```
...
```
In [217]: c1 = pd.Categorical(["b", "c"])
In [218]: c2 = pd.Categorical(["a", "b"])
In [219]: c1
Out[219]: 
['b', 'c']
...
# "b" is coded to 1
In [222]: c2.codes
Out[222]: array([0, 1], dtype=int8)
In [223]: c = union_categoricals([c1, c2])
In [224]: c
Out[224]: 
['b', 'c', 'a', 'b']
Categories (3, str): ['b', 'c', 'a']
...
In [225]: c.codes
Out[225]: array([0, 1, 2, 0], dtype=int8)
```
## Getting data in/out#
...
```
In [226]: import io
In [227]: s = pd.Series(pd.Categorical(["a", "b", "b", "a", "a", "d"]))
# rename the categories
In [228]: s = s.cat.rename_categories(["very good", "good", "bad"])
# reorder the categories and add missing categories
In [229]: s = s.cat.set_categories(["very bad", "bad", "medium", "good", "very good"])
In [230]: df = pd.DataFrame({"cats": s, "vals": [1, 2, 3, 4, 5, 6]})
In [231]: csv = io.StringIO()
In [232]: df.to_csv(csv)
In [233]: df2 = pd.read_csv(io.StringIO(csv.getvalue()))
In [234]: df2.dtypes
Out[234]: 
Unnamed: 0    int64
cats            str
...
Name: cats, dtype: str
# Redo the category
In [236]: df2["cats"] = df2["cats"].astype("category")
In [237]: df2["cats"] = df2["cats"].cat.set_categories(
...
default not included in computations. See Working with missing data.
...
```
In [240]: s = pd.Series(["a", "b", np.nan, "a"], dtype="category")
# only two categories
In [241]: s
Out[241]: 
0      a
...
```
In [243]: s = pd.Series(["a", "b", np.nan], dtype="category")
In [244]: s
Out[244]: 
0      a
1      b
...
```
In [247]: s = pd.Series(["foo", "bar"] * 1000, dtype="object")
# object dtype
In [248]: s.memory_usage(deep=True)
Out[248]: 104132
# category dtype
In [249]: s.astype("category").memory_usage(deep=True)
Out[249]: 2155
```
Note
...
```
In [250]: s = pd.Series(["foo%04d" % i for i in range(2000)], dtype="object")
# object dtype
In [251]: s.memory_usage(deep=True)
Out[251]: 112132
# category dtype
In [252]: s.astype("category").memory_usage(deep=True)
Out[252]: 34382
```
...
In [254]: dtype = pd.Categorical(["a"]).dtype
In [255]: try:
   .....:     np.dtype(dtype)
   .....: except TypeError as e:
   .....:     print("TypeError:", str(e))
...
```
In [256]: dtype == np.str_
Out[256]: False
In [257]: np.str_ == dtype
Out[257]: False
```
To check if a Series contains Categorical data, use `hasattr(s, 'cat')`:

```
In [258]: hasattr(pd.Series(["a"], dtype="category"), "cat")
Out[258]: True
In [259]: hasattr(pd.Series(["a"]), "cat")
Out[259]: False
```
...
```
In [260]: s = pd.Series(pd.Categorical([1, 2, 3, 4]))
In [261]: try:
   .....:     np.sum(s)
   .....: except TypeError as e:
   .....:     print("TypeError:", str(e))
...
In [262]: df = pd.DataFrame(
   ...
   .....: )
   .....: 
In [263]: df.apply(lambda row: type(row["cats"]), axis=1)
Out[263]: 
0    <class 'int'>
1    <class 'int'>
2    <class 'int'>
3    <class 'int'>
dtype: object
In [264]: df.apply(lambda col: col.dtype, axis=0)
Out[264]: 
a          int64
b            str
cats    category
...
```
In [265]: cats = pd.Categorical([1, 2, 3, 4], categories=[4, 2, 3, 1])
In [266]: strings = ["a", "b", "c", "d"]
In [267]: values = [4, 2, 3, 1]
In [268]: df = pd.DataFrame({"strings": strings, "values": values}, index=cats)
In [269]: df.index
Out[269]: 
CategoricalIndex([1, 2, 3, 4], categories=[4, 2, 3, 1], ordered=False,
                 dtype='category')
# This now sorts by the categories order
In [270]: df.sort_index()
Out[270]: 
  strings  values
4       d       1
2       b       2
