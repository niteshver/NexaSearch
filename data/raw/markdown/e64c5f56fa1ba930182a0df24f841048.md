...
```
- 
            Air quality dataFor this tutorial, air quality data about To raw data𝑁  𝑂 2 `air_quality_no2_long.csv"` data set provides𝑁  𝑂 2 *FR04014* ,*BETR801* and*London
...
- Plot the typical 𝑁  𝑂 2 In [14]: fig, axs = plt.subplots(figsize=(12, 4)) In [15]: air_quality.groupby(air_quality["datetime"].dt.hour)["value"].mean().plot( ....: kind='bar', rot=0, ax=axs ....: ) ....: Out[15]: <Axes: xlabel='datetime'> In [16]: plt.xlabel("Hour of the day"); # custom x label using Matplotlib In [17]: plt.ylabel("$NO_2 (µg/m^3)$"); Similar to the previous case, we want to calculate a given statistic (e.g. mean 𝑁  𝑂 2 **for each hour of the day** and we can use the
...
- Create a plot of the 𝑁  𝑂 2 In [21]: no_2["2019-05-20":"2019-05-21"].plot(); By providing a **string that parses to a datetime** , a specific subset of the data can be selected on a`DatetimeIndex` .
...
Out[24]: <MonthBegin>
```
- Make a plot of the daily mean 𝑁  𝑂 2 In [25]: no_2.resample("D").mean().plot(style="-o", figsize=(10, 5));
