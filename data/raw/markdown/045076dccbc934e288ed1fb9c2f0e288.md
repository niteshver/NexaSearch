...
- Call `func` on self producing a Series with the same axis shape as self.Unlike `agg` , which can return a scalar or a smaller result,`transform` must return a result that is the same size as the input.
This makes it suitable for element-wise operations.
  - Parameters:
    - **func** function, str, list-like or dict-like
    - Function to use for transforming the data. If a function, must either work when passed a Series or when passed to Series.apply. If func is both list-like and dict-like, dict-like behavior takes precedence. Accepted combinations are: 
      - function
