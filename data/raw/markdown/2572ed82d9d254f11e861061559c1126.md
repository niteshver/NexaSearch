...
80 ms +- 3.55 ms per loop (mean +- std. dev. of 7 runs, 10 loops each)
```
...
In [6]: %prun -l 4 df.apply(lambda x: integrate_f(x['a'], x['b'], x['N']), axis=1)
         608881 function calls (608856 primitive calls) in 0.262 seconds
   Ordered by: internal time
   List reduced from 168 to 4 due to restriction <4>
   ncalls  tottime  percall  cumtime  percall filename:lineno(function)
     1000    0.152    0.000    0.223    0.000 <ipython-input-4-c2a74e076cf0>:1(integrate_f)
   552423    0.071    0.000    0.071    0.000 <ipython-input-3-c138bdd570e3>:1(f)
     3000    0.006    0.000    0.025    0.000 series.py:1050(__getitem__)
     3000    0.005    0.000    0.011    0.000 series.py:1134(_get_value)
```
...
54.8 ms +- 5.8 ms per loop (mean +- std. dev. of 7 runs, 10 loops each)
```
...
5.15 ms +- 108 us per loop (mean +- std. dev. of 7 runs, 100 loops each)
```
...
In [12]: %prun -l 4 df.apply(lambda x: integrate_f_typed(x['a'], x['b'], x['N']), axis=1)
         55458 function calls (55433 primitive calls) in 0.019 seconds
   Ordered by: internal time
   List reduced from 166 to 4 due to restriction <4>
   ncalls  tottime  percall  cumtime  percall filename:lineno(function)
     3000    0.003    0.000    0.013    0.000 series.py:1050(__getitem__)
     3000    0.002    0.000    0.005    0.000 series.py:1134(_get_value)
    19096    0.002    0.000    0.003    0.000 {built-in method builtins.isinstance}
     3000    0.002    0.000    0.003    0.000 indexing.py:3716(check_dict_or_set_indexers)
```
```
In [13]: %%cython
   ....: cimport numpy as np
...
719 us +- 2.46 us per loop (mean +- std. dev. of 7 runs, 1,000 loops each)
```
...
In [15]: %prun -l 4 apply_integrate_f(df['a'].to_numpy(), df['b'].to_numpy(), df['N'].to_numpy())
   ...
   Ordered by: internal time
   List reduced from 46 to 4 due to restriction <4>
   ncalls  tottime  percall  cumtime  percall filename:lineno(function)
        1    0.001    0.001    0.001    0.001 <string>:1(<module>)
        1    0.000    0.000    0.000    0.000 {method 'disable' of '_lsprof.Profiler' objects}
        1    0.000    0.000    0.001    0.001 {built-in method builtins.exec}
        3    0.000    0.000    0.000    0.000 array.py:858(__getitem__)
```
```
In [16]: %%cython
   ....: cimport cython
...
605 us +- 2.63 us per loop (mean +- std. dev. of 7 runs, 1,000 loops each)
```
...
2. Define your own Python function decorated with `@jit` and pass the underlying NumPy array of`Series` or`DataFrame` (using`Series.to_numpy()` ) into the function
...
by evaluating arithmetic and boolean expressions all at once for large `DataFrame`
...
`DataFrame` with more than 100,000 rows **and** an expression
that involves multiple operations. For simple expressions
(e.g., a single comparison or arithmetic operation), `eval()`
adds overhead and will be slower regardless of `DataFrame` size.
...
Traceback (most recent call last):
  File ~/work/pandas/pandas/.pixi/envs/docs-and-web/lib/python3.12/site-packages/IPython/core/interactiveshell.py:3775 in run_code
    exec(code_obj, self.user_global_ns, self.user_ns)
  Cell In[26], line 1
    pd.eval("@a + b")
  File ~/work/pandas/pandas/pandas/core/computation/eval.py:384 in eval
    _check_for_locals(expr, level, parser)
  File ~/work/pandas/pandas/pandas/core/computation/eval.py:177 in _check_for_locals
...
7.35 ms +- 65.3 us per loop (mean +- std. dev. of 7 runs, 100 loops each)
```
```
In [41]: %timeit pd.eval("df1 + df2 + df3 + df4", engine="python")
8.15 ms +- 58.7 us per loop (mean +- std. dev. of 7 runs, 100 loops each)
```
...
3.44 ms +- 130 us per loop (mean +- std. dev. of 7 runs, 100 loops each)
```
```
In [61]: %timeit pd.eval("df1 + df2 + df3 + df4")
4.59 ms +- 56.9 us per loop (mean +- std. dev. of 7 runs, 100 loops each)
```
`DataFrame` comparison:

```
In [62]: %timeit (df1 > 0) & (df2 > 0) & (df3 > 0) & (df4 > 0)
6.09 ms +- 64.8 us per loop (mean +- std. dev. of 7 runs, 100 loops each)
```
```
In [63]: %timeit pd.eval("(df1 > 0) & (df2 > 0) & (df3 > 0) & (df4 > 0)")
10.7 ms +- 87.1 us per loop (mean +- std. dev. of 7 runs, 100 loops each)
```
...
4.72 ms +- 15.3 us per loop (mean +- std. dev. of 7 runs, 100 loops each)
```
```
In [66]: %timeit pd.eval("df1 + df2 + df3 + df4 + s")
5.78 ms +- 179 us per loop (mean +- std. dev. of 7 runs, 100 loops each)
```
Note
...
`DataFrame` size because `eval()` has parsing and compilation overhead
