...
53.8 ms +- 174 us per loop (mean +- std. dev. of 7 runs, 10 loops each)
```
...
In [6]: %prun -l 4 df.apply(lambda x: integrate_f(x['a'], x['b'], x['N']), axis=1)
         608881 function calls (608856 primitive calls) in 0.157 seconds
   Ordered by: internal time
   List reduced from 168 to 4 due to restriction <4>
   ncalls  tottime  percall  cumtime  percall filename:lineno(function)
     1000    0.091    0.000    0.135    0.000 <ipython-input-4-c2a74e076cf0>:1(integrate_f)
   552423    0.045    0.000    0.045    0.000 <ipython-input-3-c138bdd570e3>:1(f)
     3000    0.003    0.000    0.014    0.000 series.py:1048(__getitem__)
     3000    0.003    0.000    0.006    0.000 series.py:1132(_get_value)
```
...
53 ms +- 232 us per loop (mean +- std. dev. of 7 runs, 10 loops each)
```
...
6.55 ms +- 12.2 us per loop (mean +- std. dev. of 7 runs, 100 loops each)
```
...
In [12]: %prun -l 4 df.apply(lambda x: integrate_f_typed(x['a'], x['b'], x['N']), axis=1)
         55458 function calls (55433 primitive calls) in 0.021 seconds
   Ordered by: internal time
   List reduced from 166 to 4 due to restriction <4>
   ncalls  tottime  percall  cumtime  percall filename:lineno(function)
     3000    0.003    0.000    0.014    0.000 series.py:1048(__getitem__)
     3000    0.002    0.000    0.006    0.000 series.py:1132(_get_value)
    19096    0.002    0.000    0.003    0.000 {built-in method builtins.isinstance}
     3000    0.002    0.000    0.002    0.000 base.py:3743(get_loc)
```
```
In [13]: %%cython
   ....: cimport numpy as np
...
887 us +- 1.97 us per loop (mean +- std. dev. of 7 runs, 1,000 loops each)
```
...
In [15]: %prun -l 4 apply_integrate_f(df['a'].to_numpy(), df['b'].to_numpy(), df['N'].to_numpy())
   ...
   ncalls  tottime  percall  cumtime  percall filename:lineno(function)
        ...
        3    0.000    0.000    0.000    0.000 array.py:832(__getitem__)
```
```
In [16]: %%cython
   ....: cimport cython
...
665 us +- 3.02 us per loop (mean +- std. dev. of 7 runs, 1,000 loops each)
```
...
Traceback (most recent call last):
  File ~/work/pandas/pandas/.pixi/envs/docs-and-web/lib/python3.12/site-packages/IPython/core/interactiveshell.py:3748 in run_code
...
6.83 ms +- 126 us per loop (mean +- std. dev. of 7 runs, 100 loops each)
```
```
In [41]: %timeit pd.eval("df1 + df2 + df3 + df4", engine="python")
7.49 ms +- 117 us per loop (mean +- std. dev. of 7 runs, 100 loops each)
```
...
2.93 ms +- 44.8 us per loop (mean +- std. dev. of 7 runs, 100 loops each)
```
```
In [61]: %timeit pd.eval("df1 + df2 + df3 + df4")
4.07 ms +- 38.6 us per loop (mean +- std. dev. of 7 runs, 100 loops each)
```
`DataFrame` comparison:

```
In [62]: %timeit (df1 > 0) & (df2 > 0) & (df3 > 0) & (df4 > 0)
5.3 ms +- 26.8 us per loop (mean +- std. dev. of 7 runs, 100 loops each)
```
```
In [63]: %timeit pd.eval("(df1 > 0) & (df2 > 0) & (df3 > 0) & (df4 > 0)")
8.89 ms +- 45 us per loop (mean +- std. dev. of 7 runs, 100 loops each)
```
...
4.31 ms +- 95.8 us per loop (mean +- std. dev. of 7 runs, 100 loops each)
```
```
In [66]: %timeit pd.eval("df1 + df2 + df3 + df4 + s")
5.53 ms +- 40.6 us per loop (mean +- std. dev. of 7 runs, 100 loops each)
```
Note
