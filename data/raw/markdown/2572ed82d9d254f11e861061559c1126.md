...
103 ms +- 2.5 ms per loop (mean +- std. dev. of 7 runs, 10 loops each)
```
...
In [6]: %prun -l 4 df.apply(lambda x: integrate_f(x['a'], x['b'], x['N']), axis=1)
         608881 function calls (608856 primitive calls) in 0.278 seconds
   Ordered by: internal time
   List reduced from 168 to 4 due to restriction <4>
   ncalls  tottime  percall  cumtime  percall filename:lineno(function)
     1000    0.166    0.000    0.244    0.000 <ipython-input-4-c2a74e076cf0>:1(integrate_f)
   552423    0.078    0.000    0.078    0.000 <ipython-input-3-c138bdd570e3>:1(f)
     3000    0.005    0.000    0.022    0.000 series.py:1050(__getitem__)
    19096    0.004    0.000    0.005    0.000 {built-in method builtins.isinstance}
```
...
37.2 ms +- 423 us per loop (mean +- std. dev. of 7 runs, 10 loops each)
```
...
5.84 ms +- 16.2 us per loop (mean +- std. dev. of 7 runs, 100 loops each)
```
...
776 us +- 908 ns per loop (mean +- std. dev. of 7 runs, 1,000 loops each)
```
...
In [15]: %prun -l 4 apply_integrate_f(df['a'].to_numpy(), df['b'].to_numpy(), df['N'].to_numpy())
   ...
   ncalls  tottime  percall  cumtime  percall filename:lineno(function)
        ...
        3    0.000    0.000    0.000    0.000 array.py:856(__getitem__)
```
```
In [16]: %%cython
   ....: cimport cython
...
685 us +- 1.66 us per loop (mean +- std. dev. of 7 runs, 1,000 loops each)
```
...
Traceback (most recent call last):
  ...
  Cell In[26], line 1
    pd.eval("@a + b")
  File ~/work/pandas/pandas/pandas/core/computation/eval.py:363 in eval
...
8.92 ms +- 100 us per loop (mean +- std. dev. of 7 runs, 100 loops each)
```
```
In [41]: %timeit pd.eval("df1 + df2 + df3 + df4", engine="python")
9.6 ms +- 115 us per loop (mean +- std. dev. of 7 runs, 100 loops each)
```
...
4.46 ms +- 83.7 us per loop (mean +- std. dev. of 7 runs, 100 loops each)
```
```
In [61]: %timeit pd.eval("df1 + df2 + df3 + df4")
6.64 ms +- 123 us per loop (mean +- std. dev. of 7 runs, 100 loops each)
```
`DataFrame` comparison:

```
In [62]: %timeit (df1 > 0) & (df2 > 0) & (df3 > 0) & (df4 > 0)
8.26 ms +- 76.8 us per loop (mean +- std. dev. of 7 runs, 100 loops each)
```
```
In [63]: %timeit pd.eval("(df1 > 0) & (df2 > 0) & (df3 > 0) & (df4 > 0)")
10.5 ms +- 32.7 us per loop (mean +- std. dev. of 7 runs, 100 loops each)
```
...
6.33 ms +- 190 us per loop (mean +- std. dev. of 7 runs, 100 loops each)
```
```
In [66]: %timeit pd.eval("df1 + df2 + df3 + df4 + s")
7.8 ms +- 62.8 us per loop (mean +- std. dev. of 7 runs, 100 loops each)
```
Note
