...
For an overview, see Table Visualization.
...
| `Styler.format_index_names` ([formatter, axis, ...]) | Format the text display value of index names or column names. | 
...
| `Styler.set_tooltips` (ttips[, props, ...]) | Set the DataFrame of strings on `Styler` generating`:hover` tooltips. | 
...
| `Styler.to_typst` ([buf, encoding, ...]) | Write Styler to a file, buffer or string in Typst format. | 
