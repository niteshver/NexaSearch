![Hugging Face's logo](https://huggingface.co/front/assets/huggingface_logo-noborder.svg) Hugging Face⟨1⟩
  *  Models ⟨2⟩
  *  Datasets ⟨3⟩
  *  Spaces ⟨4⟩
  *  Buckets new⟨5⟩
  *  Docs ⟨6⟩
  *  Enterprise ⟨7⟩
  * Pricing⟨8⟩
  *     * Website
      *  Tasks⟨9⟩
      *  HuggingChat⟨10⟩
      *  Collections⟨11⟩
      *  Languages⟨12⟩
      *  Organizations⟨13⟩
    * Community
      *  Blog⟨14⟩
      *  Posts⟨15⟩
      *  Daily Papers⟨16⟩
      *  Hardware⟨17⟩
      *  Learn⟨18⟩
      *  Discord⟨19⟩
      *  Forum⟨20⟩
      *  GitHub⟨21⟩
    * Solutions
      *  Team & Enterprise⟨7⟩
      *  Hugging Face PRO⟨22⟩
      *  Enterprise Support⟨23⟩
      *  Inference Providers⟨24⟩
      *  Inference Endpoints⟨25⟩
      *  Storage Buckets⟨5⟩
  * * * *
  * Log In⟨26⟩
  * Sign Up⟨27⟩


# 
 ![](https://cdn-avatars.huggingface.co/v1/production/uploads/69de42156623bb0c57f1ecf6/Jg6kihL6lKXsg6GMg53p0.jpeg) ⟨28⟩
arinbalyan⟨28⟩
/
agroaid-models⟨29⟩
like 0
 PyTorch ⟨30⟩
License: mit
 Model card ⟨29⟩ Files Files and versions xet ⟨31⟩ Community ⟨32⟩
Copy to bucket new
  * AgroAid Models⟨33⟩
    * Crop Disease Classifier⟨34⟩
    * Crop Yield Predictor⟨35⟩
    * Crop Recommendation⟨36⟩


#   ⟨33⟩ AgroAid Models 
Trained models for the AgroAid smart farming application:
##   ⟨34⟩ Crop Disease Classifier 
  * Model: ResNet-18
  * Dataset: PlantVillage (54 classes, 87K images)
  * Training notebook: arinbalyan/agroaid-crop-disease-classification
  * Status: Training on Kaggle


##   ⟨35⟩ Crop Yield Predictor 
  * Model: LightGBM / Random Forest
  * Dataset: FAO Crop Yield (Kaggle)
  * Training notebook: arinbalyan/agroaid-crop-yield-prediction
  * Status: Training on Kaggle


##   ⟨36⟩ Crop Recommendation 
  * Model: XGBoost
  * Dataset: Kaggle Crop Recommendation
  * Training notebook: arinbalyan/agroaid-crop-recommendation
  * Status: Training on Kaggle


Models will be uploaded after Kaggle training completes. Downloads last month
    
-     
Downloads are not tracked for this model. How to track ⟨37⟩
Inference Providers NEW⟨38⟩
This model isn't deployed by any Inference Provider. 🙋 Ask for provider support⟨39⟩
System theme
Company
TOS⟨40⟩ Privacy⟨41⟩ About⟨42⟩ Careers⟨43⟩ ⟨1⟩
Website
Models⟨2⟩ Datasets⟨3⟩ Spaces⟨4⟩ Pricing⟨8⟩ Docs⟨6⟩
Inference providers allow you to run inference using different serverless providers.
