...
- Replace values where the condition is True. Where `cond` is True, the result takes the value from`other` ; where False, it keeps the original value. Inverse of`where()` . Useful for replacing invalid or sentinel values.
  - Parameters:
...
dtypes).
 ...
 See also 
  ...
  - `Series.where()`
  - Return an object of same shape as caller.
 Notes The mask method is an application of the if-then idiom. For each element in the caller, if `cond` is`False` the
...
index positions will be filled with True.The signature for `Series.where()` or`DataFrame.where()` differs from`numpy.where()` .
Roughly`df1.where(m, df2)` is equivalent to`np.where(m, df1, df2)` .For further details and examples see the `mask` documentation in
