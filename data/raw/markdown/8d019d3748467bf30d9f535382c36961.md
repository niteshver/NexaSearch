...
- Create an object to more easily perform multi-index slicing. `IndexSlice` is a convenience object that allows the use of natural
slice syntax (`start:stop` ) when selecting from a`MultiIndex` ,
rather than requiring explicit`slice()` calls.See also 
