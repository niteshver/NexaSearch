...
chained assignment section for more details.
...
propagating back to the parent `DataFrame`. See the
chained assignment section on how to
...
Selecting a column now returns a new `Series` object each time rather
than a cached one, so setting `Series.attrs` on a selected column
(for example `df["foo"].attrs[...] = ...`) is no longer visible on
subsequent selections of that column. Store such metadata on the parent
`DataFrame`’s `attrs` instead.
...
`Index.get_loc()` or you need to convert the row position to its index.
Both variants are shown in the following example.

```
In [36]: df = pd.DataFrame({"foo": [1, 2, 3], "bar": [4, 5, 6]})
In [37]: df.iloc[0, df.columns.get_loc("foo")] = 100
In [38]: df.loc[df.index[1], "bar"] = 200
In [39]: df
Out[39]: 
   foo  bar
0  100    4
...
mask, `DataFrame.where()` is another suitable alternative for this case.
...
```
In [40]: df = pd.DataFrame({"a": [1, 2], "b": [1.5, 2.5]})
In [41]: df.to_numpy()
Out[41]: 
array([[1. , 1.5],
       [2. , 2.5]])
...
```
In [42]: df = pd.DataFrame({"a": [1, 2], "b": [3, 4]})
In [43]: df.to_numpy()
Out[43]: 
array([[1, 3],
       [2, 4]])
...
```
In [46]: arr = df.to_numpy()
In [47]: arr.flags.writeable = True
In [48]: arr[0, 0] = 100
In [49]: arr
Out[49]: 
array([[100,   3],
...
```
In [50]: df = pd.DataFrame({"a": [1, 2, 3], "b": [4, 5, 6]})
In [51]: df2 = df.reset_index(drop=True)
In [52]: df2.iloc[0, 0] = 100
```
...
```
In [53]: df = pd.DataFrame({"a": [1, 2, 3], "b": [4, 5, 6]})
In [54]: df = df.reset_index(drop=True)
In [55]: df.iloc[0, 0] = 100
```
...
See For existing users of the nullable StringDtype for more details.
...
astype(str) preserving missing values for more details.
...
the `decode()` method now has a `dtype` parameter to be
...
In pandas < 3, calling the `prod()` method on a Series with
...
value indicator. In addition, also through `ArrowDtype` (by using
...
specifying `dtype="string"`, by using `convert_dtypes()`, or
