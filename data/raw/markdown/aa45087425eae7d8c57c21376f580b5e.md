...
- Convert the DataFrame to a dictionary. The type of the key-value pairs can be customized with the parameters (see below). 
  - Parameters:
    ...
    - Whether to include the index item (and index_names item if orient is ‘tight’) in the returned dictionary. Can only be `False` when orient is ‘split’ or ‘tight’. Note that when orient is
‘records’, this parameter does not take effect (index item always
not included).Added in version 2.0.0.
  - Returns:
