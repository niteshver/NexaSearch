...
across both axes.Added in version 2.0.0.
 ...
 Examples >>> s = pd.Series([1, 2, 2, 3], index=["cat", "dog", "dog", "mouse"]) >>> s cat 1 dog 2 dog 2 mouse 3 dtype: int64 >>> round(s.kurt(), 6) 1.5 With a DataFrame >>> df = pd.DataFrame( ... {"a": [1, 2, 2, 3], "b": [3, 4, 4, 4]}, ... index=["cat", "dog", "dog", "mouse"], ... ) >>> df a b cat 1 3 dog 2 4 dog 2 4 mouse 3 4 >>> round(df.kurt(), 6) a 1.5 b 4.0 dtype: float64 With axis=None >>> round(df.kurt(axis=None), 6) -0.988693 Using axis=1 >>> df = pd.DataFrame( ... {"a": [1, 2], "b": [3, 4], "c": [3, 4], "d": [1, 2]}, ... index=["cat", "dog"], ... ) >>> df.kurt(axis=1) cat -6.0 dog -6.0 dtype: float64