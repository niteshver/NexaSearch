...
- Set the table styles included within the `<style>` HTML element.This function can be used to style the entire table, columns, rows or specific HTML selectors. 
  - Parameters:
    ...
    - If supplying a list, each individual table_style should be a dictionary with `selector` and`props` keys.`selector` should be a CSS selector that the style will be applied to
(automatically prefixed by the table’s UUID) and`props` should be a list of tuples with`(attribute, value)` .
