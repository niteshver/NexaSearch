...
113 ms +- 5.57 ms per loop (mean +- std. dev. of 7 runs, 10 loops each)
```
...
In [6]: %prun -l 4 df.apply(lambda x: integrate_f(x['a'], x['b'], x['N']), axis=1)
         608950 function calls (608927 primitive calls) in 0.416 seconds
   Ordered by: internal time
   List reduced from 183 to 4 due to restriction <4>
   ncalls  tottime  percall  cumtime  percall filename:lineno(function)
     1000    0.251    0.000    0.360    0.000 <ipython-input-4-c2a74e076cf0>:1(integrate_f)
   552423    0.110    0.000    0.110    0.000 <ipython-input-3-c138bdd570e3>:1(f)
     3000    0.009    0.000    0.034    0.000 series.py:945(__getitem__)
     3000    0.006    0.000    0.016    0.000 series.py:1029(_get_value)
```
...
80.3 ms +- 11.6 ms per loop (mean +- std. dev. of 7 runs, 10 loops each)
```
...
13 ms +- 1.51 ms per loop (mean +- std. dev. of 7 runs, 100 loops each)
```
...
In [12]: %prun -l 4 df.apply(lambda x: integrate_f_typed(x['a'], x['b'], x['N']), axis=1)
         55527 function calls (55504 primitive calls) in 0.033 seconds
   Ordered by: internal time
   List reduced from 181 to 4 due to restriction <4>
   ncalls  tottime  percall  cumtime  percall filename:lineno(function)
     3000    0.006    0.000    0.022    0.000 series.py:945(__getitem__)
     3000    0.004    0.000    0.009    0.000 series.py:1029(_get_value)
     3000    0.003    0.000    0.004    0.000 base.py:3601(get_loc)
    19113    0.003    0.000    0.004    0.000 {built-in method builtins.isinstance}
```
```
In [13]: %%cython
   ....: cimport numpy as np
...
911 us +- 11.9 us per loop (mean +- std. dev. of 7 runs, 1,000 loops each)
```
...
685 us +- 3.54 us per loop (mean +- std. dev. of 7 runs, 1,000 loops each)
```
...
6.85 ms +- 123 us per loop (mean +- std. dev. of 7 runs, 100 loops each)
```
```
In [41]: %timeit pd.eval("df1 + df2 + df3 + df4", engine="python")
7.52 ms +- 100 us per loop (mean +- std. dev. of 7 runs, 100 loops each)
```
...
3.07 ms +- 63.1 us per loop (mean +- std. dev. of 7 runs, 100 loops each)
```
```
In [61]: %timeit pd.eval("df1 + df2 + df3 + df4")
4.42 ms +- 77.4 us per loop (mean +- std. dev. of 7 runs, 100 loops each)
```
`DataFrame` comparison:

```
In [62]: %timeit (df1 > 0) & (df2 > 0) & (df3 > 0) & (df4 > 0)
5.27 ms +- 13.5 us per loop (mean +- std. dev. of 7 runs, 100 loops each)
```
```
In [63]: %timeit pd.eval("(df1 > 0) & (df2 > 0) & (df3 > 0) & (df4 > 0)")
9.15 ms +- 39.2 us per loop (mean +- std. dev. of 7 runs, 100 loops each)
```
...
5.38 ms +- 120 us per loop (mean +- std. dev. of 7 runs, 100 loops each)
```
```
In [66]: %timeit pd.eval("df1 + df2 + df3 + df4 + s")
5.71 ms +- 72.4 us per loop (mean +- std. dev. of 7 runs, 100 loops each)
```
Note
