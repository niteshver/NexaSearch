...
- Read SQL database table into a DataFrame. Given a table name and a SQLAlchemy connectable, returns a DataFrame. This function does not support DBAPI connections. 
  - Parameters:
    - **table_name** str
...
such as SQLite.
    ...
    - If specified, returns an iterator where chunksize is the number of rows to include in each chunk. By itself this typically does not reduce peak memory usage, as most drivers buffer the full result set unless a server-side cursor is used; see the user guide on streaming results.
