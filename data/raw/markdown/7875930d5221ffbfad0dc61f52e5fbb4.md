...
- Convert columns to best possible dtypes using dtypes supporting `pd.NA` .New in version 1.0.0. 
  - Parameters
...
Series in a DataFrame) to dtypes that support`pd.NA` . By using the options`convert_string` ,`convert_integer` ,`convert_boolean` and`convert_boolean` , it is possible to turn off individual conversions
