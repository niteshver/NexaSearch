...
- Get Multiplication of dataframe and other, element-wise (binary operator rmul). Equivalent to `other * dataframe` , but with support to substitute a fill_value
for missing data in one of the inputs. With reverse version, mul.Among flexible wrappers (add, sub, mul, div, floordiv, mod, pow) to arithmetic operators: +, -, *, /, //, %, **. 
  - Parameters:
