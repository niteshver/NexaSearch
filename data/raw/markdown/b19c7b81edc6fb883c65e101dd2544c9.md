...
- `.loc` is primarily label based, but may also be used with a boolean array.`.loc` will raise`KeyError` when the items are not found. Allowed inputs are:
  - A single label, e.g. `5` or`'a'` (Note that`5` is interpreted as a*label* of the index. This use is**not** an integer position along the
index.).
...
out-of-bounds indexing.  (this conforms with Python/NumPy*slice* semantics).  Allowed inputs are:
  - An integer e.g. `5` .
...
- `.loc` ,`.iloc` , and also`[]` indexing can accept a`callable` as indexer. See more at Selection By Callable.Note Destructuring tuple keys into row (and column) indexes occurs *before* callables are applied, so you cannot return a tuple from
...
See Selection by label for details.
...
In [45]: dfl = pd.DataFrame(np.random.randn(5, 4), ....: columns=list('ABCD'), ....: index=pd.date_range('20130101', periods=5)) ....: In [46]: dfl Out[46]: A B C D 2013-01-01 1.075770 -0.109050 1.643563 -1.469388 2013-01-02 0.357021 -0.674600 -1.776904 -0.968914 2013-01-03 -1.294524 0.413738 0.276662 -0.472035 2013-01-04 -0.013960 -0.362543 -0.006154 -0.923061 2013-01-05 0.895717 0.805244 -1.206412 2.565646 In [47]: dfl.loc[2:3] --------------------------------------------------------------------------- TypeError Traceback (most recent call last) Cell In[47], line 1 ----> 1 dfl.loc[2:3] File ~/work/pandas/pandas/pandas/core/indexing.py:1288, in _LocationIndexer.__getitem__(self, key) 1286 maybe_callable = com.apply_if_callable(key, self.obj) 1287 maybe_callable = self._raise_callable_usage(key, maybe_callable) -> 1288 return self._getitem_axis(maybe_callable, axis=axis) File ~/work/pandas/pandas/pandas/core/indexing.py:1860, in _LocIndexer._getitem_axis(self, key, axis) 1858 if isinstance(key, slice): 1859 self._validate_key(key, axis) -> 1860 return self._get_slice_axis(key, axis=axis) 1861 elif com.is_bool_indexer(key): 1862 return self._getbool_axis(key, axis=axis) File ~/work/pandas/pandas/pandas/core/indexing.py:1924, in _LocIndexer._get_slice_axis(self, slice_obj, axis) 1921 return obj.copy(deep=False) 1923 labels = obj._get_axis(axis) -> 1924 indexer = labels.slice_indexer(slice_obj.start, slice_obj.stop, slice_obj.step) 1926 if isinstance(indexer, slice): 1927 return self.obj._slice(indexer, axis=axis) File ~/work/pandas/pandas/pandas/core/indexes/datetimes.py:1298, in DatetimeIndex.slice_indexer(self, start, end, step) 1290 # GH#33146 if start and end are combinations of str and None and Index is not 1291 # monotonic, we can not use Index.slice_indexer because it does not honor the 1292 # actual elements, is only searching for start and end 1293 if ( 1294 check_str_or_none(start) 1295 or check_str_or_none(end) 1296 or self.is_monotonic_increasing 1297 ): -> 1298 return Index.slice_indexer(self, start, end, step) 1300 mask = np.array(True) 1301 in_index = True File ~/work/pandas/pandas/pandas/core/indexes/base.py:7071, in Index.slice_indexer(self, start, end, step) 7020 def slice_indexer( 7021 self, 7022 start: Hashable | None = None, 7023 end: Hashable | None = None, 7024 step: int | None = None, 7025 ) -> slice: 7026 """ 7027 Compute the slice indexer for input labels and step. 7028 (...) 7069 slice(1, 3, None) 7070 """ -> 7071 start_slice, end_slice = self.slice_locs(start, end, step=step) 7073 # return a slice 7074 if not is_scalar(start_slice): File ~/work/pandas/pandas/pandas/core/indexes/base.py:7348, in Index.slice_locs(self, start, end, step) 7346 start_slice = None 7347 if start is not None: -> 7348 start_slice = self.get_slice_bound(start, "left") 7349 if start_slice is None: 7350 start_slice = 0 File ~/work/pandas/pandas/pandas/core/indexes/base.py:7234, in Index.get_slice_bound(self, label, side) 7230 original_label = label 7232 # For datetime indices label may be a string that has to be converted 7233 # to datetime boundary according to its resolution. -> 7234 label = self._maybe_cast_slice_bound(label, side) 7236 # we need to look up the label 7237 try: File ~/work/pandas/pandas/pandas/core/indexes/datetimes.py:1258, in DatetimeIndex._maybe_cast_slice_bound(self, label, side) 1249 label = Timestamp(label).to_pydatetime() 1250 warnings.warn( 1251 # GH#35830 deprecate last remaining inconsistent date treatment 1252 "Slicing with a datetime.date object is deprecated. " (...) 1255 stacklevel=find_stack_level(), 1256 ) -> 1258 label = super()._maybe_cast_slice_bound(label, side) 1259 self._data._assert_tzawareness_compat(label) 1260 return Timestamp(label) File ~/work/pandas/pandas/pandas/core/indexes/datetimelike.py:624, in DatetimeIndexOpsMixin._maybe_cast_slice_bound(self, label, side) 622 return lower if side == "left" else upper 623 elif not isinstance(label, self._data._recognized_scalars): --> 624 self._raise_invalid_indexer("slice", label) 626 return label File ~/work/pandas/pandas/pandas/core/indexes/base.py:4289, in Index._raise_invalid_indexer(self, form, key, reraise) 4287 if reraise is not lib.no_default: 4288 raise TypeError(msg) from reraise -> 4289 raise TypeError(msg) TypeError: cannot do slice indexing on DatetimeIndex with these indexers [2] of type int
...
In [73]: s.loc[10:50:5]              # (10), then skip 5 positions → 35 only
Out[73]: 
10    2
35    7
dtype: int64
...
File ~/work/pandas/pandas/pandas/core/indexing.py:2340, in _iLocIndexer._get_list_axis(self, key, axis)
   2339 try:
-> 2340     return self.obj.take(key, axis=axis)
   2341 except IndexError as err:
   2342     # re-raise with different error message, e.g. test_getitem_ndarray_3d
File ~/work/pandas/pandas/pandas/core/generic.py:4196, in NDFrame.take(self, indices, axis, **kwargs)
   4194             return self.copy(deep=False)
   4195 
-> 4196         new_data = self._mgr.take(
   4197             indices,
File ~/work/pandas/pandas/pandas/core/internals/managers.py:1091, in BaseBlockManager.take(self, indexer, axis, verify)
   1090 n = self.shape[axis]
-> 1091 indexer = maybe_convert_indices(indexer, n, verify=verify)
   1093 new_labels = self.axes[axis].take(indexer)
...
File ~/work/pandas/pandas/pandas/core/indexing.py:2369, in _iLocIndexer._getitem_axis(self, key, axis)
   2367 # a list of integers
   2368 elif is_list_like_indexer(key):
-> 2369     return self._get_list_axis(key, axis=axis)
   2371 # a single integer
   2372 else:
   2373     key = item_from_zerodim(key)
File ~/work/pandas/pandas/pandas/core/indexing.py:2343, in _iLocIndexer._get_list_axis(self, key, axis)
   2340     return self.obj.take(key, axis=axis)
   2341 except IndexError as err:
   2342     # re-raise with different error message, e.g. test_getitem_ndarray_3d
-> 2343     raise IndexError("positional indexers are out-of-bounds") from err
...
File ~/work/pandas/pandas/pandas/core/indexing.py:2316, in _iLocIndexer._getitem_tuple(self, tup)
   2315 def _getitem_tuple(self, tup: tuple):
-> 2316     tup = self._validate_tuple_indexer(tup)
   2317     with suppress(IndexingError):
   2318         return self._getitem_lowerdim(tup)
File ~/work/pandas/pandas/pandas/core/indexing.py:1070, in _LocationIndexer._validate_tuple_indexer(self, key)
   1068 for i, k in enumerate(key):
   1069     try:
-> 1070         self._validate_key(k, i)
   1071     except ValueError as err:
   1072         raise ValueError(
   1073             f"Location based indexing can only have [{self._valid_types}] types"
   1074         ) from err
File ~/work/pandas/pandas/pandas/core/indexing.py:2210, in _iLocIndexer._validate_key(self, key, axis)
   2208     return
   2209 elif is_integer(key):
-> 2210     self._validate_integer(key, axis)
   2211 elif isinstance(key, tuple):
   2212     # a tuple should already have been caught by this point
   2213     # so don't treat a tuple as a valid indexer
   2214     raise IndexingError("Too many indexers")
File ~/work/pandas/pandas/pandas/core/indexing.py:2311, in _iLocIndexer._validate_integer(self, key, axis)
   2309 len_axis = len(self.obj._get_axis(axis))
   2310 if key >= len_axis or key < -len_axis:
-> 2311     raise IndexError("single positional indexer is out-of-bounds")
...
```
In [166]: df.loc[dates[-1] + pd.Timedelta('1 day'), 0] = 7
In [167]: df
Out[167]: 
                   A         B         C         D    0
2000-01-01 -0.282863  0.469112 -1.509059 -1.135632  NaN
2000-01-02 -0.173215  1.212112  0.119209 -1.044236  NaN
...
2000-01-06  7.000000 -0.673690 -1.478427  0.524988  NaN
...
In [365]: idx2
Out[365]: 
DatetimeIndex(['2011-01-01', 'NaT', '2011-01-03'], dtype='datetime64[us]',
              freq=None)
...
This is most visible for `ExtensionDtype` columns
such as `category`, `Int64`, `boolean`, or `string`, where the
replacing form silently drops the extension dtype:

```
In [384]: df = pd.DataFrame({"a": pd.Categorical(["x", "y"], categories=["x", "y", "z"])})
In [385]: df["a"] = "z"          # replaces the column
In [386]: df.dtypes
Out[386]: 
a    str
dtype: object
In [387]: df = pd.DataFrame({"a": pd.Categorical(["x", "y"], categories=["x", "y", "z"])})
In [388]: df.loc[:, "a"] = "z"   # sets values in place
In [389]: df.dtypes
Out[389]: 
a    category
dtype: object
...
```
In [390]: df = pd.DataFrame({"a": pd.array([1, 2], dtype="Int64")})
In [391]: df["a"] = 5
In [392]: df.dtypes
Out[392]: 
a    int64
dtype: object
In [393]: df = pd.DataFrame({"a": pd.array([1, 2], dtype="Int64")})
In [394]: df.loc[:, "a"] = 5
In [395]: df.dtypes
Out[395]: 
a    Int64
