...
In [45]: dfl = pd.DataFrame(np.random.randn(5, 4), ....: columns=list('ABCD'), ....: index=pd.date_range('20130101', periods=5)) ....: In [46]: dfl Out[46]: A B C D 2013-01-01 1.075770 -0.109050 1.643563 -1.469388 2013-01-02 0.357021 -0.674600 -1.776904 -0.968914 2013-01-03 -1.294524 0.413738 0.276662 -0.472035 2013-01-04 -0.013960 -0.362543 -0.006154 -0.923061 2013-01-05 0.895717 0.805244 -1.206412 2.565646 In [47]: dfl.loc[2:3] --------------------------------------------------------------------------- TypeError Traceback (most recent call last) Cell In[47], line 1 ----> 1 dfl.loc[2:3] File ~/work/pandas/pandas/pandas/core/indexing.py:1342, in _LocationIndexer.__getitem__(self, key) 1340 maybe_callable = com.apply_if_callable(key, self.obj) 1341 maybe_callable = self._raise_callable_usage(key, maybe_callable) -> 1342 return self._getitem_axis(maybe_callable, axis=axis) File ~/work/pandas/pandas/pandas/core/indexing.py:1914, in _LocIndexer._getitem_axis(self, key, axis) 1912 if isinstance(key, slice): 1913 self._validate_key(key, axis) -> 1914 return self._get_slice_axis(key, axis=axis) 1915 elif com.is_bool_indexer(key): 1916 return self._getbool_axis(key, axis=axis) File ~/work/pandas/pandas/pandas/core/indexing.py:1978, in _LocIndexer._get_slice_axis(self, slice_obj, axis) 1975 return obj.copy(deep=False) 1977 labels = obj._get_axis(axis) -> 1978 indexer = labels.slice_indexer(slice_obj.start, slice_obj.stop, slice_obj.step) 1980 if isinstance(indexer, slice): 1981 return self.obj._slice(indexer, axis=axis) File ~/work/pandas/pandas/pandas/core/indexes/datetimes.py:1338, in DatetimeIndex.slice_indexer(self, start, end, step) 1330 # GH#33146 if start and end are combinations of str and None and Index is not 1331 # monotonic, we can not use Index.slice_indexer because it does not honor the 1332 # actual elements, is only searching for start and end 1333 if ( 1334 check_str_or_none(start) 1335 or check_str_or_none(end) 1336 or self.is_monotonic_increasing 1337 ): -> 1338 return Index.slice_indexer(self, start, end, step) 1340 mask = np.array(True) 1341 in_index = True File ~/work/pandas/pandas/pandas/core/indexes/base.py:7086, in Index.slice_indexer(self, start, end, step) 7035 def slice_indexer( 7036 self, 7037 start: Hashable | None = None, 7038 end: Hashable | None = None, 7039 step: int | None = None, 7040 ) -> slice: 7041 """ 7042 Compute the slice indexer for input labels and step. 7043 (...) 7084 slice(1, 3, None) 7085 """ -> 7086 start_slice, end_slice = self.slice_locs(start, end, step=step) 7088 # return a slice 7089 if not is_scalar(start_slice): File ~/work/pandas/pandas/pandas/core/indexes/base.py:7363, in Index.slice_locs(self, start, end, step) 7361 start_slice = None 7362 if start is not None: -> 7363 start_slice = self.get_slice_bound(start, "left") 7364 if start_slice is None: 7365 start_slice = 0 File ~/work/pandas/pandas/pandas/core/indexes/base.py:7249, in Index.get_slice_bound(self, label, side) 7245 original_label = label 7247 # For datetime indices label may be a string that has to be converted 7248 # to datetime boundary according to its resolution. -> 7249 label = self._maybe_cast_slice_bound(label, side) 7251 # we need to look up the label 7252 try: File ~/work/pandas/pandas/pandas/core/indexes/datetimes.py:1298, in DatetimeIndex._maybe_cast_slice_bound(self, label, side) 1289 label = Timestamp(label).to_pydatetime() 1290 warnings.warn( 1291 # GH#35830 deprecate last remaining inconsistent date treatment 1292 "Slicing with a datetime.date object is deprecated. " (...) 1295 stacklevel=find_stack_level(), 1296 ) -> 1298 label = super()._maybe_cast_slice_bound(label, side) 1299 self._data._assert_tzawareness_compat(label) 1300 return Timestamp(label) File ~/work/pandas/pandas/pandas/core/indexes/datetimelike.py:629, in DatetimeIndexOpsMixin._maybe_cast_slice_bound(self, label, side) 627 return lower if side == "left" else upper 628 elif not isinstance(label, self._data._recognized_scalars): --> 629 self._raise_invalid_indexer("slice", label) 631 return label File ~/work/pandas/pandas/pandas/core/indexes/base.py:4304, in Index._raise_invalid_indexer(self, form, key, reraise) 4302 if reraise is not lib.no_default: 4303 raise TypeError(msg) from reraise -> 4304 raise TypeError(msg) TypeError: cannot do slice indexing on DatetimeIndex with these indexers [2] of type int
...
File ~/work/pandas/pandas/pandas/core/indexing.py:2394, in _iLocIndexer._get_list_axis(self, key, axis)
   2393 try:
-> 2394     return self.obj.take(key, axis=axis)
   2395 except IndexError as err:
   2396     # re-raise with different error message, e.g. test_getitem_ndarray_3d
File ~/work/pandas/pandas/pandas/core/generic.py:4238, in NDFrame.take(self, indices, axis, **kwargs)
   4236             return self.copy(deep=False)
   4237 
-> 4238         new_data = self._mgr.take(
   4239             indices,
...
File ~/work/pandas/pandas/pandas/core/indexing.py:2423, in _iLocIndexer._getitem_axis(self, key, axis)
   2421 # a list of integers
   2422 elif is_list_like_indexer(key):
-> 2423     return self._get_list_axis(key, axis=axis)
   2425 # a single integer
   2426 else:
   2427     key = item_from_zerodim(key)
File ~/work/pandas/pandas/pandas/core/indexing.py:2397, in _iLocIndexer._get_list_axis(self, key, axis)
   2394     return self.obj.take(key, axis=axis)
   2395 except IndexError as err:
   2396     # re-raise with different error message, e.g. test_getitem_ndarray_3d
-> 2397     raise IndexError("positional indexers are out-of-bounds") from err
...
File ~/work/pandas/pandas/pandas/core/indexing.py:2370, in _iLocIndexer._getitem_tuple(self, tup)
   2369 def _getitem_tuple(self, tup: tuple):
-> 2370     tup = self._validate_tuple_indexer(tup)
   2371     with suppress(IndexingError):
   2372         return self._getitem_lowerdim(tup)
File ~/work/pandas/pandas/pandas/core/indexing.py:1124, in _LocationIndexer._validate_tuple_indexer(self, key)
   1122 for i, k in enumerate(key):
   1123     try:
-> 1124         self._validate_key(k, i)
   1125     except ValueError as err:
   1126         raise ValueError(
   1127             f"Location based indexing can only have [{self._valid_types}] types"
   1128         ) from err
File ~/work/pandas/pandas/pandas/core/indexing.py:2264, in _iLocIndexer._validate_key(self, key, axis)
   2262     return
   2263 elif is_integer(key):
-> 2264     self._validate_integer(key, axis)
   2265 elif isinstance(key, tuple):
   2266     # a tuple should already have been caught by this point
   2267     # so don't treat a tuple as a valid indexer
   2268     raise IndexingError("Too many indexers")
File ~/work/pandas/pandas/pandas/core/indexing.py:2365, in _iLocIndexer._validate_integer(self, key, axis)
   2363 len_axis = len(self.obj._get_axis(axis))
   2364 if key >= len_axis or key < -len_axis:
-> 2365     raise IndexError("single positional indexer is out-of-bounds")
