  ...
  - Parameters:
    ...
    - **engine_kwargs** dict, default None None
      - For `'cython'` engine, there are no accepted`engine_kwargs`
        ...
        - and `parallel` dictionary keys. The values must either be`True` or`False` . The default`engine_kwargs` for the`'numba'` engine is`{'nopython': True, 'nogil': False, 'parallel': False}` and will be
