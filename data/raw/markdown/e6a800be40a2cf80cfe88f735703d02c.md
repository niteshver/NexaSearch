...
- `Index` : the generic “ordered set” object, holding axis labels of
any supported dtype (numpy numeric dtypes,`object` ,`str` , or an`ExtensionArray` dtype). The labels must be hashable (and
likely immutable) but need not be unique. Populates a dict of label
to location in Cython to do`O(1)` lookups.
- `MultiIndex` : the standard hierarchical index object
- `DatetimeIndex` : An`Index` object backed by a`DatetimeArray` , exposing`Timestamp` boxed elements
- `TimedeltaIndex` : An`Index` object backed by a`TimedeltaArray` , exposing`Timedelta` boxed elements
- `PeriodIndex` : An`Index` object backed by a`PeriodArray` , exposing`Period` boxed elements

There are functions that make the creation of a regular index easy:

- `date_range()` : fixed frequency date range generated from a time rule or
DateOffset. Returns a`DatetimeIndex` .
- `period_range()` : fixed frequency date range generated from a time rule or
DateOffset. Returns a`PeriodIndex` of`Period` objects, representing timespans.
...
constructors `from_tuples()` and `from_arrays()` ensure
...
- `cls._values` is the “best possible” array. This could be an`ndarray` or`ExtensionArray` .
