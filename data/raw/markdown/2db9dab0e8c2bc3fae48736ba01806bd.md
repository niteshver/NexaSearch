...
and their values are fed into the rows of the `DataFrame`, and should all
...
```
In [63]: from dataclasses import make_dataclass
In [64]: Point = make_dataclass("Point", [("x", int), ("y", int)])
In [65]: pd.DataFrame([Point(0, 0), Point(0, 3), Point(2, 3)])
Out[65]: 
   x  y
0  0  0
1  0  3
...
```
In [66]: pd.DataFrame.from_dict(dict([("A", [1, 2, 3]), ("B", [4, 5, 6])]))
Out[66]: 
   A  B
0  1  4
1  2  5
...
In [69]: pd.DataFrame.from_records(data, index="C")
Out[69]: 
          A    B
C               
b'Hello'  1  2.0
...
In [71]: df["three"] = df["one"] * df["two"]
In [72]: df["flag"] = df["one"] > 2
In [73]: df
Out[73]: 
   one  two  three   flag
a  1.0  1.0    1.0  False
...
```
In [83]: iris = pd.read_csv("data/iris.data")
In [84]: iris.head()
Out[84]: 
...
In [85]: iris.assign(sepal_ratio=iris["SepalWidth"] / iris["SepalLength"]).head()
Out[85]: 
...
```
In [86]: iris.assign(sepal_ratio=lambda x: (x["SepalWidth"] / x["SepalLength"])).head()
Out[86]: 
   SepalLength  SepalWidth  PetalLength  PetalWidth         Name  sepal_ratio
0          5.1         3.5          1.4         0.2  Iris-setosa     0.686275
1          4.9         3.0          1.4         0.2  Iris-setosa     0.612245
...
```
In [87]: iris.assign(sepal_ratio=pd.col("SepalWidth") / pd.col("SepalLength")).head()
Out[87]: 
   SepalLength  SepalWidth  PetalLength  PetalWidth         Name  sepal_ratio
0          5.1         3.5          1.4         0.2  Iris-setosa     0.686275
1          4.9         3.0          1.4         0.2  Iris-setosa     0.612245
...
In [88]: (
   ...
   ....: )
   ....: 
Out[88]: <Axes: xlabel='SepalRatio', ylabel='PetalRatio'>
```
...
```
In [89]: dfa = pd.DataFrame({"A": [1, 2, 3], "B": [4, 5, 6]})
In [90]: dfa.assign(C=lambda x: x["A"] + x["B"], D=lambda x: x["A"] + x["C"])
Out[90]: 
   A  B  C   D
0  1  4  5   6
1  2  5  7   9
...
```
In [93]: df = pd.DataFrame(np.random.randn(10, 4), columns=["A", "B", "C", "D"])
In [94]: df2 = pd.DataFrame(np.random.randn(7, 3), columns=["A", "B", "C"])
In [95]: df + df2
Out[95]: 
          A         B         C   D
...
```
In [100]: df1 = pd.DataFrame({"a": [1, 0, 1], "b": [0, 1, 1]}, dtype=bool)
In [101]: df2 = pd.DataFrame({"a": [0, 1, 1], "b": [1, 1, 0]}, dtype=bool)
In [102]: df1 & df2
Out[102]: 
       a      b
0  False  False
...
```
In [111]: ser1 = pd.Series([1, 2, 3], index=["a", "b", "c"])
In [112]: ser2 = pd.Series([1, 3, 5], index=["b", "a", "c"])
In [113]: ser1
Out[113]: 
a    1
b    2
...
```
In [116]: ser3 = pd.Series([2, 4, 6], index=["b", "c", "d"])
In [117]: ser3
Out[117]: 
b    2
c    4
...
```
In [119]: ser = pd.Series([1, 2, 3])
In [120]: idx = pd.Index([4, 5, 6])
In [121]: np.maximum(ser, idx)
Out[121]: 
0    4
1    5
2    6
...
```
In [122]: baseball = pd.read_csv("data/baseball.csv")
In [123]: print(baseball)
...
```
In [125]: print(baseball.iloc[-20:, :12].to_string())
...
```
In [127]: pd.set_option("display.width", 40)  # default is 80
...
In [129]: datafile = {
   ...
   .....: }
   .....: 
In [130]: pd.set_option("display.max_colwidth", 30)
In [131]: pd.DataFrame(datafile)
Out[131]: 
      filename                           path
0  filename_01  media/user_name/storage/fo...
1  filename_02  media/user_name/storage/fo...
In [132]: pd.set_option("display.max_colwidth", 100)
...
```
In [134]: df = pd.DataFrame({"foo1": np.random.randn(5), "foo2": np.random.randn(5)})
In [135]: df
Out[135]: 
       foo1      foo2
0  1.126203  0.781836
