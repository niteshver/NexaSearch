...
- Apply a CSS-styling function column-wise, row-wise, or table-wise. Updates the HTML representation with the result. 
  - Parameters:
    - **func** function
...
valid index labels considering`subset` .`func` should take a DataFrame if`axis` is`None` and return either
