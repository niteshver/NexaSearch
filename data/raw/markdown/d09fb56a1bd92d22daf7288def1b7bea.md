...
on relative positions. For example, the weights of`adjust=True` , and`adjust=False` .
    ...
    - New in version 1.1.0. Only applicable to `mean()` .Times corresponding to the observations. Must be monotonically increasing and `datetime64[ns]` dtype.If 1-D array like, a sequence with the same shape as the observations.
    - **method** str {‘single’, ‘table’}, default ‘single’
    - New in version 1.4.0. Execute the rolling operation per single column or row ( `'single'` )
