...
- Format the text display value of index labels or column headers. This method assigns a formatting function to the index or column header labels of the DataFrame, similar to `Styler.format()` but applied
...
Escaping is done before`formatter` .
 ...
 Notes This method assigns a formatting function, `formatter` , to each level label
...
be string or callable, as above.The default formatter currently expresses floats and complex numbers with the pandas display precision unless using the `precision` argument here. The
