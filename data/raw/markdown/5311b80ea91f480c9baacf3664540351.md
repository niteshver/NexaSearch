...
```
In [26]: import glob
In [27]: import tempfile
In [28]: N = 12
In [29]: starts = [f"20{i:>02d}-01-01" for i in range(N)]
In [30]: ends = [f"20{i:>02d}-12-13" for i in range(N)]
In [31]: tmpdir = tempfile.TemporaryDirectory(ignore_cleanup_errors=True)
In [32]: for i, (start, end) in enumerate(zip(starts, ends)):
...
In [33]: %%time
   ...
   ....: counts.astype(int)
   ....: 
CPU times: user 569 ms, sys: 38.3 ms, total: 607 ms
Wall time: 379 ms
Out[33]: 
name
Alice      1994645
