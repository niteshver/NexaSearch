...
Cell In[10], line 1
----> 1 pd.get_option("max")
File ~/work/pandas/pandas/pandas/_config/config.py:188, in get_option(pat)
    144 def get_option(pat: str) -> Any:
    145     """
    146     Retrieve the value of the specified option.
    147 
   (...)    186     4
    187     """
--> 188     key = _get_single_key(pat)
    190     # walk the nested dict
    191     root, k = _get_root(key)
File ~/work/pandas/pandas/pandas/_config/config.py:134, in _get_single_key(pat)
    132     raise OptionError(f"No such keys(s): {pat!r}")
    133 if len(keys) > 1:
--> 134     raise OptionError("Pattern matched multiple keys")
    135 key = keys[0]
    137 _warn_if_deprecated(key)
...
future.distinguish_nan_and_na Whether to treat NaN entries as distinct from pd.NA in numpy-nullable and pyarrow float dtypes. By default treats both interchangeable as missing values (NaN will be coerced to NA). See discussion in https://github.com/pandas-dev/pandas/issues/32265
    [default: False] [currently: False]
future.infer_freq_returns_offset Whether pd.infer_freq and .inferred_freq return a BaseOffset object instead of a string, which will be the default in a future version of pandas. Set to True to opt in to the future behavior, or False to keep the old behavior and silence the warning.
    [default: None] [currently: None]
future.infer_string Whether to infer sequence of str objects as pyarrow string dtype, which will be the default in pandas 3.0 (at which point this option will be deprecated).
    [default: True] [currently: True]
future.no_silent_downcasting This option is deprecated and will be removed in a future version. It has no effect.
    [default: False] [currently: False]
    (Deprecated, use `` instead.)
future.python_scalars Whether to return Python scalars instead of NumPy or PyArrow scalars. Values from object dtype data where the operation coerces them to NumPy floats remain NumPy scalars. Currently experimental, setting to True is not recommended for end users.
    [default: False] [currently: False]
...
In [89]: pd.set_option("display.precision", 2)
