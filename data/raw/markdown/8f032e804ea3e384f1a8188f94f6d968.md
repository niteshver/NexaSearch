...
dtype: float64
```
The windows are formed by looking back the length of the window from the current observation. The result above can be derived by taking the sum of the following windowed partitions of data:
...
with magnitude `1/np.finfo(np.double).eps` (approximately 
...
The `FixedForwardWindowIndexer` class is available for this purpose.
...
Additionally, `apply()` can leverage Numba
...
See enhancing performance with Numba for general usage of the arguments and performance considerations.
...
passed then computes the statistic for each pair of columns, returning a`DataFrame` with a`MultiIndex` whose values are the dates in question (see the next section).
...
The default, `adjust=True`, uses the weights 
...
When `adjust=False`, we have 

One must have **span**, **center of mass (com)** or **half-life** of an EW moment:
...
in terms of span:- **Half-life** is the period of time for the exponential weight to reduce to
one half.
...
The recursion `adjust=False` assumes that no values are missing; it holds because those
...
```
In [81]: ser = pd.Series([3, np.nan, 5])
In [82]: ser.ewm(alpha=2 / 3, adjust=False, ignore_na=False).mean()
Out[82]: 
0    3.000000
1    3.000000
2    4.714286
dtype: float64
In [83]: ser.ewm(alpha=2 / 3, adjust=False, ignore_na=True).mean()
Out[83]: 
0    3.000000
1    3.000000
2    4.333333
