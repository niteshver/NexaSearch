...
File ~/work/pandas/pandas/pandas/core/strings/accessor.py:3495, in StringMethods.extract(self, pat, flags, expand)
   3492     raise ValueError("pattern contains no capture groups")
   3494 if not expand and regex.groups > 1 and isinstance(self._data, ABCIndex):
-> 3495     raise ValueError("only one regex group is supported with Index")
   3497 obj = self._data
   3498 result_dtype = _result_dtype(obj)
