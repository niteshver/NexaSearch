...
- Convert the DataFrame to a dictionary. The type of the key-value pairs can be customized with the parameters (see below). 
 ...
 New in version 1.4.0: ‘tight’ as an allowed value for the `orient` argument
    ...
    - Whether to include the index item (and index_names item if orient is ‘tight’) in the returned dictionary. Can only be `False` when orient is ‘split’ or ‘tight’.New in version 2.0.0.
  - Returns:
