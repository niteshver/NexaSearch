...
from src.seeder.url_seeder import URLSeeder
...
class SearchEnginePipeline:
    ...
    async def run(
        ...
        Args:
            stages: Specific stages to run or a list of seed URLs to crawl.
        ...
        if isinstance(stages, list) and stages and all(isinstance(item, str) and item.startswith(('http://', 'https://')) for item in stages):
            self.crawl_urls = [item.strip() for item in stages if isinstance(item, str) and item.strip()]
            stage_order = ['seeding', 'crawling']
        else:
    ...
    def _write_seed_artifacts(self, urls: List[str], output_dir: Path) -> None:
        """Write seed JSON and sitemap XML for a list of URLs."""
        output_dir.mkdir(parents=True, exist_ok=True)
        ...
        sitemap_path = output_dir / "master_seed.xml"

        with open(seeds_path, 'w', encoding='utf-8') as handle:
            json.dump({
                'metadata': {
...
class NexaSearchPipeline(SearchEnginePipeline):
    """Backward-compatible pipeline wrapper for the CLI and Streamlit UI."""
